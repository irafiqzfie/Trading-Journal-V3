...
const generatePrompt = (positions: Position[]): string => {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!positions || !Array.isArray(positions) || positions.length === 0) {
      return NextResponse.json({ error: 'No positions provided for analysis.' }, { status: 400 });
    }
    
    const prompt = generatePrompt(positions);
