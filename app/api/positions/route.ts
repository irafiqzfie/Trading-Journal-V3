...
export async function GET() {
  try {
    ...
    console.log(
      'KV_REST_API_TOKEN (first 10 chars):',
      process.env.KV_REST_API_TOKEN?.slice(0, 10) + '...'
    );

    if (!process.env.KV_REST_API_URL || !process.env.KV_REST_API_TOKEN) {
      console.error(
        'Vercel KV environment variables not found. Ensure the project is linked to a Vercel KV store in the Vercel dashboard.'
      );
    ...
    return NextResponse.json(
      {
        error:
          'Failed to retrieve data from the database. This could be due to incorrect environment variable configuration. Please check server logs.'
      },
      { status: 500 }
    );
  }
...
export async function POST(request: Request) {
  try {
    // 🔎 Debug: log env vars again for POST
    console.log('KV_REST_API_URL:', process.env.KV_REST_API_URL);
    console.log(
      'KV_REST_API_TOKEN (first 10 chars):',
      process.env.KV_REST_API_TOKEN?.slice(0, 10) + '...'
    ...
    return NextResponse.json(
      {
        error:
          'Failed to save data to the database. This could be due to incorrect environment variable configuration. Please check server logs.'
      },
      { status: 500 }
    );
  }
