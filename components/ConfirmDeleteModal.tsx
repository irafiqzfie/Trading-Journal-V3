    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50 transition-opacity duration-300 animate-fade-in" role="dialog" aria-modal="true" aria-labelledby="modal-title-delete" onClick={onCancel}>
      <div className="bg-stone-900/60 backdrop-blur-lg border border-white/10 rounded-lg shadow-2xl p-6 w-full max-w-md m-4 animate-slide-up-fade" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end pt-4 space-x-3 border-t border-white/10">
            ...
            <button type="button" onClick={onConfirm} className="px-5 py-2 bg-brand-loss text-white font-semibold rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105 hover:shadow-xl hover:bg-red-500">Delete</button>
        </div>
      </div>
    </div>
  );
